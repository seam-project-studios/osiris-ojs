...
async function handleCache(options, template) {
  ...
  if (options.cache) {
    ...
    if (!hasTemplate) {
      ...
      template = template.toString().replace(_BOM, '');
    }
  }
  else if (!hasTemplate) {
...
async function includeSource(path, options) {
  ...
  template = await fs.readFile(includePath);
  template = template.toString().replace(_BOM, '');
...
exports.render = async function (template, data, opts) {
  data = data || {};
  opts = opts || {};
  var fn;
  ...
  return await handleCache(opts, template)(data);
};
...
if (require.extensions) {
  require.extensions['.ejs'] = async function (module, filename) {
    filename = filename || /* istanbul ignore next */ module.filename;
    var options = {
          filename: filename
        , client: true
        }
      , template = await fs.readFile(filename).toString()
      , fn = exports.compile(template, options);
